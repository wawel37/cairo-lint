...
fn test_basic_match_expect_err_diagnostics() {
    ...
    5 |       let _foo = match foo {
      |  ________________-
    6 | |         Result::Ok(_) => core::panic_with_felt252(err),
    7 | |         Result::Err(x) => x,
    8 | |     };
      | |_____-
      |
...
fn test_match_with_function_diagnostics() {
    ...
    warning: Plugin diagnostic: Manual match for `expect_err` detected. Consider using `expect_err()` instead
     --> lib.cairo:6:16
      |
    6 |       let _foo = match foo(0) {
      |  ________________-
    7 | |         Result::Ok(_) => core::panic_with_felt252('error'),
    8 | |         Result::Err(err) => err,
    9 | |     };
      | |_____-
      |
    ");
...
fn test_if_with_function_diagnostics() {
    ...
    warning: Plugin diagnostic: Manual match for `expect_err` detected. Consider using `expect_err()` instead
      --> lib.cairo:6:14
       |
     6 |       let _a = if let Result::Err(err) = foo(0) {
       |  ______________-
     7 | |         err
     8 | |     } else {
     9 | |         core::panic_with_felt252('panic')
    10 | |     };
       | |_____-
       |
    ");
